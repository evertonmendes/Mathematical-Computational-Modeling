...
import math
import csv
from Calculate import StandardDeviation
from PLOT import PlotGenetic
...
class GA(object):
    ...
    def evolve_population(self, init_pop):
        ...
        # Mutates all the routes 
        for route in descendant_pop.rt_pop:
            self.mutate(route)

        for route in descendant_pop.rt_pop:
            self.inversion(route)

        for route in descendant_pop.rt_pop:
            self.transposition(route)
...
class App(object):
    ...
    def GA_loop(self,n_generations,pop_size, graph=False):
        ...
        for x in range(1,n_generations):
            ...
            # save the  shorter fittest lengh of each generation
            list_fittest_population.append(the_population.fittest.length)

            #Calculate the standard deviation of my routes
            value=0.0
            routesValues=[]
            for i in the_population.rt_pop:

                value=i.length
                routesValues.append(value)

            Deviation=StandardDeviation(routesValues)
        ...
        PlotGenetic(list_fittest_population, list_Deviation)
